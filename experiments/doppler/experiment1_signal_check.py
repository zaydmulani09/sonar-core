...
def run_motion_trial(trial_num):
    ...
    print(f"Trial {trial_num}: rest {rest_result['bins']} bins, "
          ...
          f"{increase_bins} bins (~{increase_hz:.1f}Hz) above rest baseline")
...
def write_results_md(motion_trials, rest_trials, passing, verdict):
    lines = []
...
def main():
    ...
    for i in range(1, N_MOTION_TRIALS + 1):
        motion_trials.append(run_motion_trial(i))
    ...
    results_path = write_results_md(motion_trials, rest_trials, n_pass, verdict)
